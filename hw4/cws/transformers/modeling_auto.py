...
MODEL_FOR_TOKEN_FEATURE_MAPPING = OrderedDict(
    [
        (BertConfig, BertForTokenFeature),
    ]
)
...
class AutoModelForTokenFeature:
    r"""
        :class:`~transformers.AutoModelForTokenFeature` is a generic model class
        that will be instantiated as one of the token feature extraction model classes of the library
        when created with the `AutoModelForTokenFeature.from_pretrained(pretrained_model_name_or_path)`
        class method.
    ...
    def __init__(self):
        raise EnvironmentError(
            "AutoModelForTokenFeature is designed to be instantiated "
            "using the `AutoModelForTokenFeature.from_pretrained(pretrained_model_name_or_path)` or "
            "`AutoModelForTokenFeature.from_config(config)` methods."
        )
    ...
    def from_config(cls, config):
        ...
        Args:
            config (:class:`~transformers.PretrainedConfig`):
                ...
                - isInstance of `distilbert` configuration class: :class:`~transformers.DistilBertModelForTokenFeature` (DistilBERT model)
                - isInstance of `xlm` configuration class: :class:`~transformers.XLMForTokenFeature` (XLM model)
                - isInstance of `xlm roberta` configuration class: :class:`~transformers.XLMRobertaModelForTokenFeature` (XLMRoberta model)
                - isInstance of `bert` configuration class: :class:`~transformers.BertModelForTokenFeature` (Bert model)
                - isInstance of `albert` configuration class: :class:`~transformers.AlbertForTokenFeature` (AlBert model)
                - isInstance of `xlnet` configuration class: :class:`~transformers.XLNetModelForTokenFeature` (XLNet model)
                - isInstance of `camembert` configuration class: :class:`~transformers.CamembertModelForTokenFeature` (Camembert model)
                - isInstance of `roberta` configuration class: :class:`~transformers.RobertaModelForTokenFeature` (Roberta model)
                - isInstance of `electra` configuration class: :class:`~transformers.ElectraForTokenFeature` (Electra model)

        Examples::

            config = BertConfig.from_pretrained('bert-base-uncased')    # Download configuration from S3 and cache.
            model = AutoModelForTokenFeature.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        for config_class, model_class in MODEL_FOR_TOKEN_FEATURE_MAPPING.items():
            if isinstance(config, config_class):
                return model_class(config)
        ...
        raise ValueError(
            ...
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_TOKEN_FEATURE_MAPPING.keys()),
            )
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        The model class to instantiate is selected as the first pattern matching
        in the `pretrained_model_name_or_path` string (in the following order):
            - contains `distilbert`: :class:`~transformers.DistilBertForTokenFeature` (DistilBERT model)
            - contains `xlm`: :class:`~transformers.XLMForTokenFeature` (XLM model)
            - contains `xlm-roberta`: :class:`~transformers.XLMRobertaForTokenFeature` (XLM-RoBERTa?Para model)
            - contains `camembert`: :class:`~transformers.CamembertForTokenFeature` (Camembert model)
            - contains `bert`: :class:`~transformers.BertForTokenFeature` (Bert model)
            - contains `xlnet`: :class:`~transformers.XLNetForTokenFeature` (XLNet model)
            - contains `roberta`: :class:`~transformers.RobertaForTokenFeature` (Roberta model)
            - contains `electra`: :class:`~transformers.ElectraForTokenFeature` (Electra model)
        ...
        Examples::
            ...
            model = AutoModelForTokenFeature.from_pretrained('bert-base-uncased')    # Download model and configuration from S3 and cache.
            model = AutoModelForTokenFeature.from_pretrained('./test/bert_model/')  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
            assert model.config.output_attention == True
            # Loading from a TF checkpoint file instead of a PyTorch model (slower)
            config = AutoConfig.from_json_file('./tf_model/bert_tf_model_config.json')
            model = AutoModelForTokenFeature.from_pretrained('./tf_model/bert_tf_checkpoint.ckpt.index', from_tf=True, config=config)
